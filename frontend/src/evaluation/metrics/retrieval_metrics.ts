...
export function calculate_mrr(retrieval_evaluation_case_results: RetrievalEvaluationCaseResult[]) {
    ...
    let dense_recall = null;
    let lexical_recall = null;
    let hybrid_recall = null;
    let reranking_recall = null;
    ...
    for (const caseResult of retrieval_evaluation_case_results) { // answer and rawretrievedresults
        ...
        if (gold_section) {
            ...
            dense_recall = calculate_recall(gold_section, caseResult.dense_results);
            lexical_recall = calculate_recall(gold_section, caseResult.lexical_results);
            hybrid_recall = calculate_recall(gold_section, caseResult.hybrid_results);
            reranking_recall = calculate_recall(gold_section, caseResult.reranking_results);
        }
    }
