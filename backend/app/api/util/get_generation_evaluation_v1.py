...
2. Partially supported: The generated answer expresses part of the nugget, but misses or contradicts part of the fact required by this nugget.
...
rules = """
Rules:
1. For each judgment, evaluate ONLY the current nugget.
   If the complete fact in the nugget is clearly expressed in the generated answer,
   label it "supported".
   Ignore all other information in the generated answer unless it directly
   contradicts this nugget.

2. If part of the fact in the nugget is expressed, but an important part is
   missing or contradicted, label it "partially supported".

3. If the fact in the nugget is not expressed at all, label it "not supported".
   Being about the same topic is NOT enough for partial support.
